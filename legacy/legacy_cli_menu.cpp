// Legacy numbered-menu CLI, superseded by the full-screen ConsoleTUI.
//
// This file is not part of the build (the Makefile only compiles src/*.cpp).
// It is kept here, rather than deleted, because it documents an earlier
// interaction model for the simulator: a classic "type a number, press
// enter" menu instead of the current live dashboard. main.cpp originally
// called into these functions directly; once ConsoleTUI took over as the
// primary interface, main() stopped calling any of them, but the functions
// stayed behind unreferenced. If you want a non-interactive, scriptable
// text menu again (e.g. for a low-bandwidth SSH session where a full-screen
// TUI redraw is too heavy), this is the starting point - it still compiles
// against the current Simulator/OutputFormatter/IOHelpers APIs as of the
// commit that moved it here.
...
// ---------------------------------------------------------------------------
// Live monitor (formerly Simulator::start_live_monitor / stop_live_monitor)
// ---------------------------------------------------------------------------
// This was a background thread that woke up every N seconds and printed a
// network summary directly to stdout - "Option 17" in the old menu above.
// It was never called from anywhere reachable in the current build (main()
// only launches the TUI or the headless benchmark), and it shared the same
// bug class as the Logger race documented in Utils.h/ConsoleTUI.cpp: it
// wrote straight to stdout via IOHelpers from its own thread, with no
// coordination with whatever else might be rendering the terminal at the
// time. The ConsoleTUI dashboard now covers the same "live network state"
// need without a second writer. Kept here as a free function (adapted from
// the original Simulator member function, using only Simulator's public
// interface) in case a non-TUI, print-to-log-file monitoring mode is wanted
// later - in that case, route it through Logger::info() instead of raw
// IOHelpers calls so it doesn't reintroduce the same race.
#include <atomic>
#include <chrono>
...
namespace {
  std::thread legacyMonitorThread;
  std::atomic<bool> legacyMonitorRun{false};
  std::condition_variable legacyMonitorCv;
  std::mutex legacyMonitorMutex;
}

void start_live_monitor(Simulator &sim, int interval_seconds = 5) {
  if (legacyMonitorRun.load()) {
    Logger::warning("Monitor is already running!");
    return;
  }
  legacyMonitorRun.store(true);
  legacyMonitorThread = std::thread([&sim, interval_seconds]() {
    while (legacyMonitorRun.load()) {
      try {
        IOHelpers::printNewline();
        IOHelpers::print("[LIVE MONITOR UPDATE]");
        IOHelpers::printNewline();

        int towerCount = sim.get_tower_count();
        IOHelpers::print("  Total Towers:  ");
        IOHelpers::print(towerCount);
        IOHelpers::printNewline();
        IOHelpers::print("  Total Devices: ");
        IOHelpers::print(sim.get_device_count());
        IOHelpers::printNewline();

        for (int i = 0; i < towerCount; ++i) {
          auto t = sim.get_tower(i);
          if (!t) continue;
        ...
        IOHelpers::println("[LiveMonitor] Error occurred during update");
      }

      std::unique_lock<std::mutex> lk(legacyMonitorMutex);
      legacyMonitorCv.wait_for(lk, std::chrono::seconds(interval_seconds),
                               []() { return !legacyMonitorRun.load(); });
    }
  });
}

void stop_live_monitor() {
  if (!legacyMonitorRun.load()) return;
  legacyMonitorRun.store(false);
  legacyMonitorCv.notify_all();
  if (legacyMonitorThread.joinable()) {
    legacyMonitorThread.join();
  }
}
